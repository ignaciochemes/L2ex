defmodule L2E.Commons.Blowfish do
  @moduledoc """
  Pure Elixir Blowfish-ECB cipher (BouncyCastle/L2J little-endian variant).

  Replaces `:crypto.crypto_one_time(:blowfish_ecb, ...)` which is unavailable
  on OpenSSL 3.x systems. Implements the same algorithm as
  `org.l2jmobius.commons.crypt.BlowfishEngine` (BouncyCastle-derived):

  - Data blocks are read/written in **little-endian** byte order.
  - Key material is mixed in **big-endian** order (standard Blowfish).
  - 16-round Feistel with output swap: bytes 0-3 carry `xr`, bytes 4-7 carry `xl`.

  Usage:
      ctx = L2E.Commons.Blowfish.init_key(key_binary)
      ciphertext = L2E.Commons.Blowfish.encrypt_ecb(plaintext, ctx)
      plaintext  = L2E.Commons.Blowfish.decrypt_ecb(ciphertext, ctx)
  """
  ...
  # ---------------------------------------------------------------------------
  # Blowfish constants: P-array (18) and S-boxes S0-S3 (4 × 256)
  # Derived from fractional digits of π — identical to BouncyCastle / L2J.
  # ---------------------------------------------------------------------------

  @kp {0x243F6A88, 0x85A308D3, 0x13198A2E, 0x03707344, 0xA4093822, 0x299F31D0, 0x082EFA98,
       0xEC4E6C89, 0x452821E6, 0x38D01377, 0xBE5466CF, 0x34E90C6C, 0xC0AC29B7, 0xC97C50DD,
       0x3F84D5B5, 0xB5470917, 0x9216D5D9, 0x8979FB1B}

  @ks0 {0xD1310BA6, 0x98DFB5AC, 0x2FFD72DB, 0xD01ADFB7, 0xB8E1AFED, 0x6A267E96, 0xBA7C9045,
  ...
  @ks1 {0x4B7A70E9, 0xB5B32944, 0xDB75092E, 0xC4192623, 0xAD6EA6B0, 0x49A7DF7D, 0x9CEE60B8,
  ...
  @ks2 {0xE93D5A68, 0x948140F7, 0xF64C261C, 0x94692934, 0x411520F7, 0x7602D4F7, 0xBCF46B2E,
  ...
  @ks3 {0x3A39CE37, 0xD3FAF5CF, 0xABC27737, 0x5AC52D1B, 0x5CB0679E, 0x4FA33742, 0xD3822740,
  ...
  @doc """
  Compute the Blowfish expanded key schedule from a key binary (1–56 bytes).
  Returns `{p, s0, s1, s2, s3}` — a 5-tuple of tuples used by `encrypt_ecb/2`
  and `decrypt_ecb/2`. Expensive (~2 ms); cache the result per connection.
  """
  @spec init_key(binary()) :: {tuple(), tuple(), tuple(), tuple(), tuple()}
  def init_key(key) when is_binary(key) and byte_size(key) >= 1 do
    p = xor_key_into_p(@kp, key)
    ctx = {p, @ks0, @ks1, @ks2, @ks3}

    {new_p, ctx} = process_table(0, 0, p, 18, ctx, 0)

    {_, ctx} = process_table(elem(new_p, 16), elem(new_p, 17), @ks0, 256, ctx, 1)
    new_s0 = elem(ctx, 1)

    {_, ctx} = process_table(elem(new_s0, 254), elem(new_s0, 255), @ks1, 256, ctx, 2)
    new_s1 = elem(ctx, 2)

    {_, ctx} = process_table(elem(new_s1, 254), elem(new_s1, 255), @ks2, 256, ctx, 3)
    new_s2 = elem(ctx, 3)

    {_, ctx} = process_table(elem(new_s2, 254), elem(new_s2, 255), @ks3, 256, ctx, 4)

    ctx
  end

  @doc """
  Encrypt `data` in ECB mode using a precomputed context from `init_key/1`.
  `data` must be a multiple of 8 bytes.
  """
  @spec encrypt_ecb(binary(), {tuple(), tuple(), tuple(), tuple(), tuple()}) :: binary()
  def encrypt_ecb(data, ctx) when rem(byte_size(data), 8) == 0 do
    for <<block::binary-8 <- data>>, into: <<>>, do: encrypt_block(block, ctx)
  end

  @doc """
  Decrypt `data` in ECB mode using a precomputed context from `init_key/1`.
  `data` must be a multiple of 8 bytes.
  """
  @spec decrypt_ecb(binary(), {tuple(), tuple(), tuple(), tuple(), tuple()}) :: binary()
  def decrypt_ecb(data, ctx) when rem(byte_size(data), 8) == 0 do
    for <<block::binary-8 <- data>>, into: <<>>, do: decrypt_block(block, ctx)
  end

  # ---------------------------------------------------------------------------
  # Block-level encrypt / decrypt (L2J little-endian variant)
  # ---------------------------------------------------------------------------

  # Read xl from bytes 0-3 LE, xr from bytes 4-7 LE.
  # Write out_xr to bytes 0-3 LE, out_xl to bytes 4-7 LE (output swap, per L2J).
  defp encrypt_block(<<xl::little-32, xr::little-32>>, ctx) do
    {out_xr, out_xl} = encrypt_pair(xl, xr, ctx)
    <<out_xr::little-32, out_xl::little-32>>
  end

  defp decrypt_block(<<xl::little-32, xr::little-32>>, ctx) do
    {out_xr, out_xl} = decrypt_pair(xl, xr, ctx)
    <<out_xr::little-32, out_xl::little-32>>
  end
  ...
  defp f(x, s0, s1, s2, s3) do
    a = elem(s0, bsr(x, 24) &&& 0xFF)
    b = elem(s1, bsr(x, 16) &&& 0xFF)
    c = elem(s2, bsr(x, 8) &&& 0xFF)
    d = elem(s3, x &&& 0xFF)
    bxor(a + b &&& @mask, c) + d &&& @mask
  end

  # ---------------------------------------------------------------------------
  # 16-round Feistel encrypt (P[0..17])
  # ---------------------------------------------------------------------------

  defp encrypt_pair(xl, xr, {p, s0, s1, s2, s3}) do
    xl = bxor(xl, elem(p, 0))
    xr = bxor(xr, bxor(f(xl, s0, s1, s2, s3), elem(p, 1)))
    xl = bxor(xl, bxor(f(xr, s0, s1, s2, s3), elem(p, 2)))
    xr = bxor(xr, bxor(f(xl, s0, s1, s2, s3), elem(p, 3)))
    xl = bxor(xl, bxor(f(xr, s0, s1, s2, s3), elem(p, 4)))
    xr = bxor(xr, bxor(f(xl, s0, s1, s2, s3), elem(p, 5)))
    xl = bxor(xl, bxor(f(xr, s0, s1, s2, s3), elem(p, 6)))
    xr = bxor(xr, bxor(f(xl, s0, s1, s2, s3), elem(p, 7)))
    xl = bxor(xl, bxor(f(xr, s0, s1, s2, s3), elem(p, 8)))
    xr = bxor(xr, bxor(f(xl, s0, s1, s2, s3), elem(p, 9)))
    xl = bxor(xl, bxor(f(xr, s0, s1, s2, s3), elem(p, 10)))
    xr = bxor(xr, bxor(f(xl, s0, s1, s2, s3), elem(p, 11)))
    xl = bxor(xl, bxor(f(xr, s0, s1, s2, s3), elem(p, 12)))
    xr = bxor(xr, bxor(f(xl, s0, s1, s2, s3), elem(p, 13)))
    xl = bxor(xl, bxor(f(xr, s0, s1, s2, s3), elem(p, 14)))
    xr = bxor(xr, bxor(f(xl, s0, s1, s2, s3), elem(p, 15)))
    xl = bxor(xl, bxor(f(xr, s0, s1, s2, s3), elem(p, 16)))
    xr = bxor(xr, elem(p, 17))
    {xr, xl}
  end

  # ---------------------------------------------------------------------------
  # 16-round Feistel decrypt (P[17..0])
  # ---------------------------------------------------------------------------

  defp decrypt_pair(xl, xr, {p, s0, s1, s2, s3}) do
    xl = bxor(xl, elem(p, 17))
    xr = bxor(xr, bxor(f(xl, s0, s1, s2, s3), elem(p, 16)))
    xl = bxor(xl, bxor(f(xr, s0, s1, s2, s3), elem(p, 15)))
    xr = bxor(xr, bxor(f(xl, s0, s1, s2, s3), elem(p, 14)))
    xl = bxor(xl, bxor(f(xr, s0, s1, s2, s3), elem(p, 13)))
    xr = bxor(xr, bxor(f(xl, s0, s1, s2, s3), elem(p, 12)))
    xl = bxor(xl, bxor(f(xr, s0, s1, s2, s3), elem(p, 11)))
    xr = bxor(xr, bxor(f(xl, s0, s1, s2, s3), elem(p, 10)))
    xl = bxor(xl, bxor(f(xr, s0, s1, s2, s3), elem(p, 9)))
    xr = bxor(xr, bxor(f(xl, s0, s1, s2, s3), elem(p, 8)))
    xl = bxor(xl, bxor(f(xr, s0, s1, s2, s3), elem(p, 7)))
    xr = bxor(xr, bxor(f(xl, s0, s1, s2, s3), elem(p, 6)))
    xl = bxor(xl, bxor(f(xr, s0, s1, s2, s3), elem(p, 5)))
    xr = bxor(xr, bxor(f(xl, s0, s1, s2, s3), elem(p, 4)))
    xl = bxor(xl, bxor(f(xr, s0, s1, s2, s3), elem(p, 3)))
    xr = bxor(xr, bxor(f(xl, s0, s1, s2, s3), elem(p, 2)))
    xl = bxor(xl, bxor(f(xr, s0, s1, s2, s3), elem(p, 1)))
    xr = bxor(xr, elem(p, 0))
    {xr, xl}
  end
  ...
  # XOR P[i] with 4 big-endian key bytes each, cycling through key.
  defp xor_key_into_p(p, key) do
    key_list = :binary.bin_to_list(key)
    key_len = length(key_list)

    {new_p, _} =
      Enum.reduce(0..17, {p, 0}, fn i, {acc_p, ki} ->
        {word, new_ki} = read_be32(key_list, key_len, ki)
        {put_elem(acc_p, i, bxor(elem(acc_p, i), word)), new_ki}
      end)

    new_p
  end

  # Read 4 bytes from key (cycling) and assemble as big-endian 32-bit word.
  defp read_be32(key_list, key_len, start_idx) do
    Enum.reduce(0..3, {0, start_idx}, fn _j, {acc, ki} ->
      byte = Enum.at(key_list, rem(ki, key_len))
      {(acc <<< 8 ||| byte) &&& @mask, ki + 1}
    end)
  end

  # Encrypt table entries in pairs, updating ctx[slot] after each pair so that
  # subsequent encrypt_pair calls see the incrementally modified table (required
  # by the Blowfish key schedule — mirrors Java's processTable mutating in-place).
  defp process_table(xl, xr, table, size, ctx, slot) do
    do_process(0, div(size, 2), xl, xr, table, ctx, slot)
  end

  defp do_process(i, n, _xl, _xr, table, ctx, _slot) when i >= n, do: {table, ctx}

  defp do_process(i, n, xl, xr, table, ctx, slot) do
    {out_xr, out_xl} = encrypt_pair(xl, xr, ctx)
    s = i * 2
    new_table = table |> put_elem(s, out_xr) |> put_elem(s + 1, out_xl)
    new_ctx = put_elem(ctx, slot, new_table)
    do_process(i + 1, n, out_xr, out_xl, new_table, new_ctx, slot)
  end
end
