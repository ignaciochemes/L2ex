...
| M72-B – Pets full | Pet inventory GenServer + `RequestPetItemList`; unsummon on owner death; pet equipment slot |
...
| M103-B – Class transfer quests (remaining races) | Quest scripts for Dark Elf, Orc, and Dwarf 1st and 2nd class transfers using existing `Quest.Engine` DSL |
| M105 – Cursed Weapons | `CursedWeapon` GenServer per weapon (Zariche / Akamanah); possession, PK accumulation, drop on death, decay timer; `ExCursedWeaponLocation` packet |
