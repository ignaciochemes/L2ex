...
| M121 – Party Room | `L2E.Party.Room` GenServer for party matching/listing; `RequestPartyMatchConfig` + `RequestPartyMatchList` client packets; `ExPartyRoomInfo` server packet; party room registry + cleanup on party form |
| M122 – Clan Skills | Clan skill requirements (clan level gate); `RequestPledgeSkillList` + `RequestAcquirePledgeSkill` client packets; `ExPledgeSkillList` server packet; `clan_skills` DB table; skill bonuses broadcast to online members via PubSub |
